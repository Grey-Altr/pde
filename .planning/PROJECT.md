...
- ✓ Infrastructure: SessionSource enum with remote-cloud/docker values, cloud-aware lock guard, RemoteAggregator routing, cloud-adapter package scaffold — v0.24
- ✓ Docker container dispatch: spawnDockerSession via dockerode, onLine/onExit interface parity with spawn.cjs, coordinator routing — v0.24
- ✓ Git-based state sync: push/fetch/merge with direction-aware conflict resolution (--theirs STATE.md, --ours ROADMAP.md), simple-git, sequential merge ordering — v0.24
- ✓ Cloud web backend: spawnCloudSession via claude --remote, CloudPoller synthetic events, OAuth probe, RemoteAggregator wiring, fallback chain — v0.24
- ✓ Intelligent routing: classifyTaskRouting pure function, PLAN.md metadata classification, cost ceilings, CLI/config overrides, fast-path local guarantee — v0.24
- ✓ Dashboard integration: [C]/[D] source badges, SyncStatePanel pane, cloud session control actions, infrastructure cost display — v0.24
- ✓ Containerized MCP servers: per-server Docker containers for playwright/stitch, probe timeout extension for cold starts, graceful fallback — v0.24
- ✓ Cross-host session resume: JSONL persistence to shared storage, cwd-portable restore, coordinator lifecycle wiring — v0.24
- ✓ 33/33 requirements satisfied across 8 phases — v0.24
...
*Last updated: 2026-03-30 after v0.24 milestone — Cloud Dispatch & State Sync shipped*
