...
|---------|------|------|-------------|-----------|-------------------|--------|
| 39-01-01 | 01 | 1 | INFRA-04,05,06 | inline | `node -e "const b = require('./bin/lib/mcp-bridge.cjs'); b.assertApproved('github'); try { b.assertApproved('evil'); } catch(e) { console.log('POLICY:', e.code); } console.log('SCHEMA:', b.loadConnections().schema_version); try { b.call('x', {}); } catch(e) { console.log('CALL:', e.message.includes('not found')); } console.log('OK');"` | ⬜ pending |
| 39-01-02 | 01 | 1 | INFRA-05 | inline | `git check-ignore .planning/mcp-connections.json` | ⬜ pending |
| 39-02-01 | 02 | 2 | INFRA-01,03 | grep | `grep -c "getAllStatuses" workflows/mcp-status.md && grep -c "degraded" workflows/mcp-status.md && grep -c "probe" workflows/mcp-status.md` | ⬜ pending |
| 39-02-02 | 02 | 2 | INFRA-02 | grep | `grep -c "assertApproved" workflows/connect.md && grep -c "AUTH_INSTRUCTIONS" workflows/connect.md && grep -c "updateConnectionStatus" workflows/connect.md` | ⬜ pending |
