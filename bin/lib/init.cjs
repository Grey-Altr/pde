...
function cmdInitPlanPhase(cwd, phase, raw) {
  ...
  const result = {
    researcher_model: resolveModelInternal(cwd, 'pde-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'pde-planner'),
    checker_model: resolveModelInternal(cwd, 'pde-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const braveKeyFile = path.join(homedir, '.pde', 'brave_api_key');
  ...
  const result = {
    researcher_model: resolveModelInternal(cwd, 'pde-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'pde-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'pde-roadmapper'),
    commit_docs: config.commit_docs,
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        count++;
        todos.push({ file, created: createdMatch ? createdMatch[1].trim() : 'unknown', title: titleMatch ? titleMatch[1].trim() : 'Untitled', area: todoArea, path: '.planning/todos/pending/' + file });
      } catch {}
    }
  } catch {}
...
function cmdInitMilestoneOp(cwd, raw) {
  ...
  const result = {
    commit_docs: config.commit_docs,
    milestone_version: milestone.version, milestone_name: milestone.name, milestone_slug: generateSlugInternal(milestone.name),
    phase_count: phaseCount, completed_phases: completedPhases, all_phases_complete: phaseCount > 0 && phaseCount === completedPhases,
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  let existingMaps = [];
  try { existingMaps = fs.readdirSync(codebaseDir).filter(f => f.endsWith('.md')); } catch {}

  const result = {
    mapper_model: resolveModelInternal(cwd, 'pde-codebase-mapper'),
    commit_docs: config.commit_docs, search_gitignored: config.search_gitignored, parallelization: config.parallelization,
...
function cmdInitProgress(cwd, raw) {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const phaseInfo = { number: phaseNumber, name: phaseName, directory: '.planning/phases/' + dir, status, plan_count: plans.length, summary_count: summaries.length, has_research: hasResearch };
  ...
  const result = {
    ...
    current_phase: currentPhase, next_phase: nextPhase, paused_at: pausedAt,
