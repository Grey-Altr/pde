---
name: pde-plan-checker
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /pde:plan-phase orchestrator.
...
Spawned by `/pde:plan-phase` orchestrator (after planner creates PLAN.md) or re-verification (after planner revises).
...
<upstream_input>
**CONTEXT.md** (if exists) — User decisions from `/pde:discuss-phase`
...
**Question:** Do plans honor user decisions from /pde:discuss-phase?
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/pde:plan-phase {N} --research` to regenerate."
...
Skip if: The current phase has no `depends_on` entry in ROADMAP.md (null or empty). Still write DEPENDENCY-GAPS.md with result: pass, gap_count: 0.

**DEPS-06 scope constraint:** ONLY check direct depends_on phases listed in the current phase's ROADMAP.md entry. Do NOT recurse into transitive dependencies.

**Process:**

1. Read the current phase number from the verification context (extracted from `phase_number` in the init JSON or from the phase directory name).

2. Run the roadmap analyze command to get the full phases array:
```bash
ROADMAP_JSON=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" roadmap analyze --raw)
```

3. Extract the current phase's `depends_on` field from the JSON:
```bash
CURRENT_DEPENDS=$(echo "$ROADMAP_JSON" | jq -r --arg ph "$PHASE_NUMBER" '.phases[] | select(.number == $ph) | .depends_on // empty')
```
The `depends_on` field is a raw string from ROADMAP.md (e.g., "Phase 54" or "Phase 55, Phase 56") or null.

4. Parse phase numbers from the `depends_on` string using the pattern `Phase\s+(\d+)` applied globally. Extract all numeric phase IDs.

5. For each extracted upstream phase number, look up its `disk_status` in the `ROADMAP_JSON` phases array:
```bash
UPSTREAM_STATUS=$(echo "$ROADMAP_JSON" | jq -r --arg upph "$UPSTREAM_PHASE" '.phases[] | select(.number == $upph) | .disk_status // "no_directory"')
```

6. Classify each upstream phase:
   - `disk_status === "complete"` → no gap (PASS)
   - `disk_status === "partial"` → CONCERNS gap (partially built — may have needed output)
   - All other statuses (`planned`, `researched`, `discussed`, `empty`, `no_directory`) → FAIL gap (nothing built yet, blocking)

7. DEPS-06: Stop after checking direct depends_on phases. Do NOT recurse to their dependencies.

8. Write DEPENDENCY-GAPS.md to the phase directory (always — even for PASS result):
```bash
cat > "$PHASE_DIR/${PHASE}-DEPENDENCY-GAPS.md" << 'GAPS_EOF'
...
**Issue output format (emitted per gap):**
```yaml
issue:
  plan: null
  dimension: "cross_phase_dependencies"
  severity: "{blocker | concerns}"  # blocker for planned/empty/researched/discussed/no_directory; concerns for partial
...
- All other statuses (`planned`, `researched`, `discussed`, `empty`, `no_directory`) → severity: "blocker"

**Output:**
```
## Dimension 9: Cross-Phase Dependencies

DEPS-06 scope: direct depends_on only (no transitive recursion)

| Upstream Phase | disk_status | Severity | Gap |
|----------------|-------------|----------|-----|
| Phase {N} | complete | PASS | No |

DEPENDENCY-GAPS.md: written to {phase_dir}
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
**passed:** All requirements covered, all tasks complete, dependency graph valid, key links planned, scope within budget, must_haves properly derived, and no cross-phase dependency gaps.

**issues_found:** One or more blockers or warnings. Plans need revision.

Dimension 9 (Cross-Phase Dependencies) gaps contribute directly to overall status: a FAIL gap from an unbuilt upstream phase is a blocker that prevents execution from succeeding. A CONCERNS gap (partial upstream) should be reviewed before proceeding.
...
Plans verified. Run `/pde:execute-phase {phase}` to proceed.
```
