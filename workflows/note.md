...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/pde:note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
{count} active note(s). Use `/pde:note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a GSD project. Run `/pde:new-project` to initialize one."
...
<edge_cases>
1. **"list" as note text**: `/pde:note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `$HOME/.claude/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/pde:new-project`
