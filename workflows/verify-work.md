<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /pde:plan-phase --gaps.
...
Provide a phase number to start testing (e.g., /pde:verify-work 4)
```
...
```
UI_PHASE_FLAG=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```
⚠ Security enforcement enabled — /pde:secure-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/pde:secure-phase {phase}` — security review (required before advancing)
...
- `/pde:plan-phase {next}` — Plan next phase
- `/pde:execute-phase {next}` — Execute next phase
- `/pde:secure-phase {phase}` — security review
- `/pde:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
<downstream_consumer>
Output consumed by /pde:execute-phase
...
Spawn pde-planner with revision context:
...
3. Abandon (exit, user runs /pde:plan-phase manually)
...
- [ ] If issues: pde-planner creates fix plans (gap_closure mode)
- [ ] If issues: pde-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/pde:execute-phase --gaps-only` when complete
</success_criteria>
