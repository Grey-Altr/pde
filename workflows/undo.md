...
```
Usage: /pde:undo --last N | --phase NN | --plan NN-MM
...
```
Working tree has uncommitted changes. Commit or stash them before running /pde:undo.
```
...
**Also available:**
- `/pde:execute-phase ${PHASE}` — re-execute if needed
- `/pde:undo --last 1` — undo the revert itself if something went wrong
