...
```bash
INIT=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /pde:new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /pde:new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /pde:new-project    # Initialize GSD in the workspace
```
...
- options:
  - "Yes — run /pde:new-project" → tell user to `cd $TARGET_PATH` first, then run `/pde:new-project`
