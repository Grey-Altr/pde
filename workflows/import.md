...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via pde-plan-checker
...
Delegate validation to pde-plan-checker:
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs({phase}): import plan from {basename FILEPATH}" --files .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `pde-plan-checker` and `pde-planner`
