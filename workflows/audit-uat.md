...
```bash
AUDIT=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" audit-uat --raw)
```
...
1. **Close stale items:** `/pde:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/pde:verify-work {phase}`
```
</step>
