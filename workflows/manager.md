...
Steps:
1. Read the plan-phase workflow: cat $HOME/.claude/pde-os/engines/gsd/workflows/plan-phase.md
2. Run: node \"$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs\" init plan-phase {N}
...
5. Spawn a pde-planner subagent via Task() to create the plans.
6. If plan-checker is enabled, spawn a pde-plan-checker subagent to verify.
...
Steps:
1. Read the execute-phase workflow: cat $HOME/.claude/pde-os/engines/gsd/workflows/execute-phase.md
2. Run: node \"$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs\" init execute-phase {N}
3. Follow the workflow steps: discover plans, analyze dependencies, group into waves.
4. For each wave, spawn pde-executor subagents via Task() to execute plans in parallel.
5. After all waves complete, spawn a pde-verifier subagent if verifier is enabled.
