...
```bash
INIT=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" init manager)
...
- `manager_flags.discuss` — appended to `/pde:discuss-phase` args (e.g. `"--auto --analyze"`)
...
All {phase_count} phases done. Ready for final steps:
  → /pde:verify-work — run acceptance testing
  → /pde:complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="pde-verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="pde-complete-milestone")` then exit.
...
```
Skill(skill="pde-discuss-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
Skill(skill=\"pde-plan-phase\", args=\"{N} --auto {manager_flags.plan}\")
...
Skill(skill=\"pde-execute-phase\", args=\"{N} {manager_flags.execute}\")
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="pde-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="pde-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="pde-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="pde-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /pde:manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/pde:manager` or `/pde:progress` invocation.
