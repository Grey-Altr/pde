...
```
Task(subagent_type="pde-assumptions-analyzer", prompt="""
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
- `/pde:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
