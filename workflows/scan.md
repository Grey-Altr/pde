<purpose>
Lightweight codebase assessment. Spawns a single pde-codebase-mapper agent for one focus area,
...
```bash
INIT=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" init map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `pde-codebase-mapper` agent with the selected focus area:
...
Use `/pde:map-codebase` for a comprehensive 4-area parallel scan.
```
