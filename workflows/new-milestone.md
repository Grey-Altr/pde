...
**Persist choice to config** (so future `/pde:plan-phase` honors it):

```bash
# If "Research first": persist true
node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" config-set workflow.research true

# If "Skip research": persist false
node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" config-set workflow.research false
```
