...
Seeds auto-surface during /pde:new-milestone when trigger conditions match the
...
This seed should be presented during `/pde:new-milestone` when the milestone
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /pde:new-milestone
