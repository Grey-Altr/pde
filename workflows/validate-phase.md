...
```bash
AUDITOR_MODEL=$(node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" resolve-model pde-nyquist-auditor --raw)
NYQUIST_CFG=$(node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" config get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /pde:settings."

Display banner: `PDE > VALIDATE PHASE {N}: {name}`
...
node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
PDE > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /pde:audit-milestone
```

**Partial:**
```
PDE > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /pde:validate-phase {N}
```
