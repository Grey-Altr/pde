...
for dir in .claude .config/opencode .opencode .gemini; do
  if [ -f "$HOME/$dir/pde/VERSION" ]; then
...
(Paths are relative to your install location: `${CLAUDE_PLUGIN_ROOT}/` for global, `./.claude/` for local)
...
**If LOCAL install:**
```bash
npx -y pde-engine-pde@latest --local
```

**If GLOBAL install (or unknown):**
```bash
npx -y pde-engine-pde@latest --global
```
