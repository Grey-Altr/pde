<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates pde-phase-researcher, pde-planner, and pde-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@$HOME/.claude/pde-os/engines/gsd/references/revision-loop.md
...
SECURITY_ASVS=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```
Skill(skill="pde-ui-phase", args="${PHASE} --auto ${GSD_WS}")
```
After `pde-ui-phase` returns, re-read:
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
**If "Split":** Use `/pde:insert-phase` to create the sub-phases, then replan each.
...
Task(
  ...
  subagent_type="pde-plan-checker",
  model="{checker_model}",
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/pde:debug` to investigate root causes."
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```
Skill(skill="pde-execute-phase", args="${PHASE} --auto --no-transition ${GSD_WS}")
```
...
- [ ] pde-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] pde-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] pde-plan-checker spawned with CONTEXT.md
