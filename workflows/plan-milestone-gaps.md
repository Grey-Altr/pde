<purpose>
Create all phases necessary to close gaps identified by `/pde:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/pde:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/pde:audit-milestone` first.
```
...
HIGHEST=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" phases list --pick directories[-1])
```
...
**Also available:**
- `/pde:execute-phase {N}` — if plans already exist
...
`/pde:audit-milestone` — re-audit to verify gaps closed
`/pde:complete-milestone {version}` — archive when audit passes
```
