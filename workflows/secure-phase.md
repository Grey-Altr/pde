...
- pde-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" agent-skills pde-security-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" resolve-model pde-security-auditor --raw)
...
If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /pde:settings."
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /pde:execute-phase {N} first."
...
## 5. Spawn pde-security-auditor

```
Task(
  prompt="Read $HOME/.claude/agents/pde:security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="pde-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `$HOME/.claude/pde-os/engines/gsd/templates/SECURITY.md`
...
▶ Fix mitigations then re-run: /pde:secure-phase {N}
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
▶ /pde:validate-phase {N}    validate test coverage
▶ /pde:verify-work {N}       run UAT
```
