...
@$HOME/.claude/pde-os/engines/gsd/references/questioning.md
@$HOME/.claude/pde-os/engines/gsd/references/domain-probes.md
</required_reading>

<available_agent_types>
Valid GSD subagent types (use exact names — do not fall back to 'general-purpose'):
- pde-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
- **Phases:** Use existing `/pde:add-phase` command via SlashCommand

Commit if `commit_docs` is enabled:
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/pde:explore` or start working with `/pde:next`.
```
