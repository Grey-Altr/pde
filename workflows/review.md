...
Write to a temp file: `/tmp/pde:review-prompt-{phase}.md`
</step>
...
**Gemini:**
```bash
gemini -p "$(cat /tmp/pde:review-prompt-{phase}.md)" 2>/dev/null > /tmp/pde:review-gemini-{phase}.md
```

**Claude (separate session):**
```bash
claude -p "$(cat /tmp/pde:review-prompt-{phase}.md)" 2>/dev/null > /tmp/pde:review-claude-{phase}.md
```

**Codex:**
```bash
codex exec --skip-git-repo-check "$(cat /tmp/pde:review-prompt-{phase}.md)" 2>/dev/null > /tmp/pde:review-codex-{phase}.md
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/pde:review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
cat /tmp/pde:review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/pde:review-opencode-{phase}.md
if [ ! -s /tmp/pde:review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/pde:review-opencode-{phase}.md
fi
```
...
Commit:
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/pde:plan-phase --reviews)
</success_criteria>
