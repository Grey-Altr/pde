...
```bash
# Get state snapshot
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```
No GSD project detected. Run `/pde:new-project` to get started.
```
Exit.
</step>
...
Run `/pde:health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
- If file exists and count >= 6: prompt "You've called /pde:next {N} times consecutively. Continue? [y/N]"
- If user says no, exit
- Increment the counter
- The counter file is deleted by any non-`/pde:next` command (convention — other workflows don't need to implement this, the note here is sufficient)
</step>
...
→ Next action: `/pde:discuss-phase <first-phase>`
...
▶ **Next step:** `/pde:[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/pde:next` is zero-friction advancement.
</step>
