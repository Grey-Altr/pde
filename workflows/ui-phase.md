<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates pde-ui-researcher and pde-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
```
UI phase is disabled in config. Enable via /pde:settings.
```
Exit workflow.
...
Recommended: run /pde:discuss-phase {N} first to capture design preferences.
...
Task(
  ...
  subagent_type="pde-ui-researcher",
...
- Re-spawn pde-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /pde:ui-phase
...
- [ ] pde-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] pde-ui-checker spawned with UI-SPEC.md
