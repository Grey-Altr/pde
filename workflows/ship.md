...
```bash
CONFIG=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
- /pde:complete-milestone (if last phase in milestone)
- /pde:progress (to see what's next)
...
- /pde:complete-milestone — if all phases in milestone are done
- /pde:progress — see overall project state
- /pde:execute-phase {next} — continue to next phase
</offer_next>
