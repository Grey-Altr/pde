...
```bash
INIT=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" init map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" agent-skills pde-codebase-mapper 2>/dev/null)
```
...
Spawn 4 parallel pde-codebase-mapper agents.

Use Task tool with `subagent_type="pde-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `pde-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="pde-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `pde-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/pde:map-codebase`
...
- If Task tool available: 4 parallel pde-codebase-mapper agents spawned with run_in_background=true
