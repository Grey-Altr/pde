...
```bash
INIT=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/pde:list-workspaces` to show available workspaces, then ask:
