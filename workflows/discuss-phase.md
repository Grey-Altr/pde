...
Use /pde:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /pde:plan-phase {X} to replan
...
**Also available:**
- `/pde:plan-phase ${PHASE} --skip-research` — plan without research
- `/pde:ui-phase ${PHASE}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
Display banner:
```bash
node "${CLAUDE_PLUGIN_ROOT}/lib/ui/render.cjs" banner "AUTO-ADVANCING TO PLAN"
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /pde:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /pde:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /pde:plan-phase ${PHASE} --gaps
  ```
