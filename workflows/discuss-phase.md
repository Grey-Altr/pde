...
<required_reading>
@$HOME/.claude/pde-os/engines/gsd/references/domain-probes.md
@$HOME/.claude/pde-os/engines/gsd/references/gate-prompts.md
@$HOME/.claude/pde-os/engines/gsd/references/universal-anti-patterns.md
</required_reading>
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/pde:discuss-phase --text`)
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/pde:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" agent-skills pde-advisor 2>/dev/null)
```
...
Use /pde:progress ${GSD_WS} to see available phases.
```
Exit workflow.
...
- Read and execute @$HOME/.claude/pde-os/engines/gsd/workflows/discuss-phase-power.md end-to-end
...
- options:
  - "Continue and replan after" — Capture context, then run /pde:plan-phase {X} ${GSD_WS} to replan
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$HOME/.claude/pde-os/engines/gsd/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" resolve-model pde-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @$HOME/.claude/agents/pde:advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
This is a structured checkpoint, not the final CONTEXT.md — the `write_context` step still produces the canonical output. But if the session dies, the next `/pde:discuss-phase` invocation can detect this checkpoint and offer to resume from it instead of starting from scratch.
...
`/pde:plan-phase ${PHASE} ${GSD_WS}`

---

**Also available:**
- `/pde:discuss-phase ${PHASE} --chain ${GSD_WS}` — re-run with auto plan+execute after
- `/pde:plan-phase ${PHASE} --skip-research ${GSD_WS}` — plan without research
- `/pde:ui-phase ${PHASE} ${GSD_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```
Skill(skill="pde-plan-phase", args="${PHASE} --auto ${GSD_WS}")
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Next: /pde:discuss-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${GSD_WS}
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /pde:execute-phase ${PHASE} ${GSD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /pde:plan-phase ${PHASE} ${GSD_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /pde:plan-phase ${PHASE} --gaps ${GSD_WS}
  ```
...
**Full step-by-step instructions:** @$HOME/.claude/pde-os/engines/gsd/workflows/discuss-phase-power.md
