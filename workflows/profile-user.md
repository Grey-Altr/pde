...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing gsd-tools.cjs subcommands and the pde-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
```bash
cp "$HOME/.claude/pde-os/engines/gsd/USER-PROFILE.md" "$HOME/.claude/pde-os/engines/gsd/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/pde-os/engines/gsd/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /pde:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn pde-user-profiler agent using Task tool:**

Use the Task tool to spawn the `pde-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/pde-os/engines/gsd/references/user-profiling.md`
...
Reference: @$HOME/.claude/pde-os/engines/gsd/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/pde-os/engines/gsd/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/pde:dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/pde-os/engines/gsd/USER-PROFILE.md" and jump to step 10.
...
**For /pde:dev-preferences (if selected):**

```bash
node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /pde:dev-preferences at $HOME/.claude/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
...
```bash
BACKUP_PATH="$HOME/.claude/pde-os/engines/gsd/USER-PROFILE.backup.md"
```
...
Your profile:    $HOME/.claude/pde-os/engines/gsd/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /pde:dev-preferences   $HOME/.claude/commands/gsd/dev-preferences.md
