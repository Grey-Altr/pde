...
| `manager.flags.discuss` | `""` | Flags passed to `/pde:discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" commit "docs: update state" --files .planning/STATE.md
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/pde:discuss-phase` from manager (e.g., `"--auto --analyze"`) |
