...
```bash
PHASE_INFO=$(node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "${CLAUDE_PLUGIN_ROOT}/bin/pde-tools.cjs" find-phase "${PHASE}" --raw)
```
