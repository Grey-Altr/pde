...
```bash
PHASE_INFO=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.claude/pde-os/engines/gsd/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
