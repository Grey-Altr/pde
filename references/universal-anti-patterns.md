...
10. **NEVER** use non-GSD agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "gsd-{agent}"` (e.g., `pde-phase-researcher`, `pde-executor`, `pde-planner`). GSD agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
