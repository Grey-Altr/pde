...
// artifact-preview.ts uses `import fs from 'node:fs/promises'` (default import),
// so we must set mocks on the `default` export.
...
// Helper: check if an argument is a ResourceTemplate for ui://pde/
function isArtifactTemplate(a: any): boolean {
  return (
    a !== null &&
    typeof a === 'object' &&
    a.constructor?.name === 'ResourceTemplate' &&
    (a.uriTemplate?.template ?? '').includes('ui://pde/')
  );
}

// Helper: find the read callback for the pde-design-artifact ResourceTemplate
function findArtifactReadCallback(
  registerResourceCalls: any[][],
  ...
  for (const call of registerResourceCalls) {
    ...
    const hasTemplate = call.some(isArtifactTemplate);
...
describe('artifact resource template — RUI-03', () => {
  ...
  it('registers a resource with uri pattern ui://pde/{artifact}', async () => {
    ...
    expect(artifactResourceCall).toBeDefined();
  });

  it('resource read callback returns text/html;profile=mcp-app MIME type', async () => {
    const fs = await import('node:fs/promises');
    // artifact-preview uses default import — mock on the default object
    (fs.default.readFile as any).mockResolvedValue('# Test Markdown\n\nHello world');

    const { registerArtifactPreviewTools } = await import('../lib/mcp/apps/artifact-preview');
    registerArtifactPreviewTools(mockServer as any);

    const readCallback = findArtifactReadCallback(mockServer.registerResource.mock.calls);
  ...
  it('resource read callback includes _meta.ui.csp.connectDomains', async () => {
    const fs = await import('node:fs/promises');
    (fs.default.readFile as any).mockResolvedValue('test content');
  ...
  it('wraps markdown content in HTML with marked rendering', async () => {
    const fs = await import('node:fs/promises');
    (fs.default.readFile as any).mockResolvedValue('# Hello\n\nWorld');
  ...
  it('renders JSON artifacts with syntax highlighting in pre block', async () => {
    const fs = await import('node:fs/promises');
    (fs.default.readFile as any).mockResolvedValue('{"key": "value"}');
  ...
  it('returns error HTML when artifact file not found', async () => {
    const fs = await import('node:fs/promises');
    (fs.default.readFile as any).mockRejectedValue(new Error('ENOENT'));
